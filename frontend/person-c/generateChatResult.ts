// ============================================================
// CHAT RESULT GENERATION (Mock AI)
// ============================================================
...
//   Replace generateChatResult() with a call to Person B's endpoint:
...
// "I can tell you're tired. That's okay. I'll stay with you — just 10 minutes."
// THEN the micro-task list appears.

// ------ Study: Low motivation / stuck ----------------------------------------

const studyLowMotivationResult: ChatResult = {
  reply:
    "I can tell you're tired and stuck right now — that's completely okay. " +
  ...
  check_in_message: "Come back after 10 minutes. Even if you only did one step — that counts.",
  check_in_options: ['Done', 'Partly done', 'I got stuck'],
  memory_update: 'User starts better with short, gentle 10-minute sprints when feeling stuck.',
  memory: {
    preferred_mode: 'Encourage Mode',
    preferred_task_length: '10-minute sprint',
    recent_goals: ['Finance quiz preparation'],
    latest_memory_update: 'User starts better with short, gentle 10-minute sprints when feeling stuck.',
    updated_at: new Date().toISOString(),
  },
  trace: [
    { step: 'Input received',     status: 'complete',  summary: '"I have a finance quiz but I am tired and stuck."' },
    { step: 'State detected',     status: 'complete',  summary: 'Low motivation + difficulty starting' },
...
const studyOverwhelmedResult: ChatResult = {
  ...
  check_in_options: ['Done', 'Partly done', 'I got stuck'],
  memory_update: 'User prefers 5-minute starts when feeling overwhelmed — smaller entry points.',
  memory: {
    preferred_mode: 'Encourage Mode',
...
const studyFocusResult: ChatResult = {
  ...
  check_in_options: ['Done', 'Partly done', 'I got stuck'],
  memory_update: 'User enters deep focus well with structured 25-minute Pomodoro sessions.',
  memory: {
...
const studyDefaultResult: ChatResult = {
  ...
  mode: 'Check-in Mode',
  goal_understanding: 'Starting a new session — checking in to understand what you need.',
  micro_task_plan: [
...
const acgResult: ChatResult = {
  ...
  check_in_options: ['Done', 'Partly done', 'I got stuck'],
  memory_update: 'User prefers calm, gentle encouragement and quiet companionship.',
  memory: {
...
const petResult: ChatResult = {
  ...
  mode: 'Routine Mode',
  goal_understanding: 'Routine wellness check-in — a short reset before continuing.',
  micro_task_plan: [
...
function detectStudyIntent(message: string): 'overwhelmed' | 'low_motivation' | 'focus' | 'default' {
...
export function generateChatResult(scenario: Scenario, userMessage: string): ChatResult {
  if (scenario === 'study') {
...
// Async version — simulate network delay (swap body for real API call later)
